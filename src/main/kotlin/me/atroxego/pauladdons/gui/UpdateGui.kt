...
class UpdateGui(restartNow: Boolean) : GuiScreen() {
    ...
    private fun doUpdate(restartNow: Boolean) {
        try {
            val directory = File(PaulAddons.modDir, "updates")
            val url = UpdateManager.updateDownloadURL
            val jarName = UpdateManager.getJarNameFromUrl(url)
            thread(name = "PaulAddons-update-downloader-thread") {
                downloadUpdate(url, directory)
                if (!failed) {
                    UpdateManager.scheduleCopyUpdateAtShutdown(jarName)
                    if (restartNow) {
                        mc.shutdown()
                    }
                    complete = true
